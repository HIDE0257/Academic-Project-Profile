...
a = Cl_SlopeLine(1)/(1+((57.3*Cl_SlopeLine(1))/(pi*e*AR))); %finding a
alpha0 = (-1)*Cl_SlopeLine(2)/Cl_SlopeLine(1);%finding alpha at 0
alpha = FP_Langle;
wing_3dCl = a * (alpha - alpha0);%final 3d wing Cl calculation

%3d wing cd
CD_wing = (FP_Cd_aoa + wing_3dCl.^2/(pi*e*AR))';

%aircraft cd
C_fe = 0.003;
%calculated
CDmin = C_fe*(S_wet/S_ref);
...
CD_aircraft = CDmin + k *(wing_3dCl - Cl_air).^2;
CD_aircraft = CD_aircraft';
clcd=wing_3dCl./CD_aircraft;
...
hold on
legend('2d truth', 'best fit', '3d flat plate');
...
legend('2d Flat Plate Wing','3d Flat Plate Wing','3D Flat Plate Aircraft')
grid on
